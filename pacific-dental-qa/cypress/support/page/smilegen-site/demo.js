class Demo {
  ...
  salesforceModalTitle = ".modal__body h1.cmp-title__text";
  ...
  quoteComponentTextCSS(quoteComponentTextFont, fontFamily, fontSize) {
    cy.get(quoteComponentTextFont).eq(0).should(
      "have.css",
      "font-family",
      fontFamily
    );
    cy.get(quoteComponentTextFont).eq(0).should("have.css", "font-size", fontSize);
  }
  ...
  quoteNameTitleAuthor(QuoteTextParagrapgh) {
    ...
    cy.get(QuoteTextParagrapgh).should("be.visible");
  ...
  titleLineColour(commonConcernTitle) {
    cy.get(commonConcernTitle).then(($el) => {
      ...
      expect(bgColor).to.eq("rgb(255, 205, 71)");
    });
  }
  ...
  mapDropdownVisible() {
    ...
    cy.get(this.mapComponentDropdown).should("exist");
  }
